      ...
      <div className="bg-white border border-slate-200/90 rounded-2xl flex flex-col shadow-xs overflow-hidden">
        ...
        <div className="p-3 sm:p-4 border-b border-slate-100 flex flex-col md:flex-row gap-3 sm:gap-4 items-center justify-between bg-slate-50/50">
          ...
          <div className="flex items-center gap-2 w-full md:w-auto overflow-x-auto pb-1 md:pb-0 hide-scrollbar">
            ...
            <button className="flex items-center gap-2 px-3 py-2 bg-white border border-slate-200 rounded-xl text-xs font-bold text-slate-700 hover:bg-slate-50 uppercase tracking-wider transition-colors shadow-xs whitespace-nowrap">
              All status <ChevronDown className="w-3.5 h-3.5 text-slate-400" />
            </button>
          </div>
        </div>
        ...
        <div className="md:hidden divide-y divide-slate-100">
          {filteredPatients && filteredPatients.length > 0 ? (
              ...
              return (
                <div key={item._id || Math.random()} className="p-4 space-y-3 bg-white hover:bg-slate-50/50 transition-colors">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <h4 className="font-bold text-slate-900 text-sm capitalize truncate">{item?.full_name}</h4>
                        <p className="text-xs text-slate-500 font-medium truncate">{item?.phone || "No phone"}</p>
                      </div>
                    </div>
                    {item?.blood_group ? (
                      <span className="status-badge active shrink-0">{item.blood_group}</span>
                    ) : (
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs pt-1 border-t border-slate-100">
                    <div>
                      <span className="text-slate-400 font-medium block text-[10px] uppercase">Age / Joined</span>
                      <span className="font-semibold text-slate-700">{age} • {joinedDate}</span>
                    </div>
                    <div>
                      <span className="text-slate-400 font-medium block text-[10px] uppercase">Doctor</span>
                      <span className="font-semibold text-slate-700 truncate block">{item?.assigned_doctor_name || "Unassigned"}</span>
                    </div>
                  </div>
            ...
            <div className="text-center py-12 text-slate-500 bg-slate-50/50 p-4">
              ...
              <span className="text-xs font-bold uppercase tracking-wider">No patients found {searchValue ? `for "${searchValue}"` : ""}</span>
            </div>
          )}
        </div>
        ...
        <div className="hidden md:block overflow-x-auto">
          <table className="w-full text-left border-collapse saas-table">
            <thead>
              <tr>
                <th className="w-12"><input type="checkbox" className="border-slate-300 text-indigo-600 rounded-sm" /></th>
                <th>Patient</th>
                <th>Contact</th>
                <th>Age</th>
                <th>BLOOD</th>
              ...
              {filteredPatients && filteredPatients.length > 0 ? filteredPatients.map((item) => {
                ...
                return (
                  <tr key={item._id || Math.random()} className="group">
                    <td><input type="checkbox" className="border-slate-300 text-indigo-600 rounded-sm" /></td>
                    <td>
        ...
        <div className="p-3 sm:p-4 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs font-bold uppercase tracking-wider text-slate-500 bg-slate-50/50">
          <div>Showing 1-{filteredPatients.length} of {filteredPatients.length}</div>
